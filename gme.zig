...
pub const Type = opaque {
	...
	pub fn emu(self: *const Type, sample_rate: u32) Error!*Emu {
	...
	/// Returns an error if out of memory.
	pub fn emuMultiChannel(self: *const Type, sample_rate: u32) Error!*Emu {
		return gme_new_emu_multi_channel(self, @intCast(sample_rate)) orelse Error.NewEmu;
	}
	extern fn gme_new_emu_multi_channel(*const Type, c_int) ?*Emu;

	/// Create an info-only emulator.
	pub fn emuInfo(self: *const Type) !*Emu {
		return gme_new_emu(self, info_only) orelse Error.NewEmu;
	}

	/// Get corresponding music type for file path or extension passed in.
	pub fn fromExtension(path_or_extension: [*:0]const u8) ?*const Type {
	...
	extern fn gme_identify_file([*:0]const u8, *?*const Type) Result;

	/// Name of game system for this music file type.
	pub fn system(self: *const Type) [:0]const u8 {
	...
	extern fn gme_type_multitrack(*const Type) c_int;
	...
	pub fn extension(music_type: *const Type) [:0]const u8 {
		const s = gme_type_extension(music_type);
		return s[0..std.mem.len(s) :0];
	}
	extern fn gme_type_extension(music_type: *const Type) [*:0]const u8;

	/// Return the fixed track count of an emu file type.
	pub fn trackCount(self: *const Type) u32 {
		return @intCast(gme_fixed_track_count(self));
	}
	extern fn gme_fixed_track_count(*const Type) c_int;
...
pub const Emu = opaque {
	...
	pub fn fromFile(path: [*:0]const u8, srate: i32) Error!*Emu {
		var self: ?*Emu = null;
		const err = gme_open_file(path, &self, @intCast(srate));
		return if (err != 0) toError(err) else self.?;
	}
	extern fn gme_open_file([*:0]const u8, *?*Emu, c_int) Result;

	/// Same as `fromFile()`, but uses file data already in memory. Makes copy of data.
	pub fn fromData(data: []const anyopaque, srate: i32) Error!*Emu {
		var self: ?*Emu = null;
		const err = gme_open_data(data.ptr, data.len, &self, @intCast(srate));
		return if (err != 0) toError(err) else self.?;
	}
	extern fn gme_open_data(*const anyopaque, c_long, *?*Emu, c_int) Result;

	/// Number of tracks available.
	pub fn trackCount(self: *const Emu) u32 {
		return @intCast(gme_track_count(self));
	}
	extern fn gme_track_count(*const Emu) c_int;

	/// Start a track, where 0 is the first track.
	pub fn startTrack(self: *Emu, index: u32) Error!void {
		const err = gme_start_track(self, @intCast(index));
		if (err != 0)
			return toError(err);
	}
	extern fn gme_start_track(*Emu, c_int) Result;
	...
	extern fn gme_set_autoload_playback_limit(*Emu, c_int) void;
	...
	extern fn gme_autoload_playback_limit(*const Emu) c_int;

	/// True if a track has reached its end.
	pub fn trackEnded(self: *const Emu) bool {
		return (gme_track_ended(self) != 0);
	}
	extern fn gme_track_ended(*const Emu) c_int;

	/// Number of milliseconds (1000 = one second) played since beginning of track.
	pub fn tell(self: *const Emu) u32 {
		return @intCast(gme_tell(self));
	}
	extern fn gme_tell(*const Emu) c_int;

	/// Number of samples generated since beginning of track.
	pub fn tellSamples(self: *const Emu) u32 {
		return @intCast(gme_tell_samples(self));
	}
	extern fn gme_tell_samples(*const Emu) c_int;

	/// Seek to new time in track. Seeking backwards or far forward can take a while.
	pub fn seek(self: *Emu, msec: u32) Error!void {
		const err = gme_seek(self, @intCast(msec));
		if (err != 0)
			return toError(err);
	}
	extern fn gme_seek(*Emu, c_int) Result;

	/// Equivalent to restarting track then skipping n samples
	pub fn seekSamples(self: *Emu, samples: u32) Error!void {
		const err = gme_seek_samples(self, @intCast(samples));
		if (err != 0)
			return toError(err);
	}
	extern fn gme_seek_samples(*Emu, c_int) Result;
	...
	pub fn warning(self: *Emu) ?[:0]const u8 {
		const warn = gme_warning(self);
		return if (warn) |s| s[0..std.mem.len(s)] else null;
	}
	extern fn gme_warning(*Emu) ?[*:0]const u8;
	...
	extern fn gme_load_m3u(*Emu, [*:0]const u8) Result;

	/// Gets information for a particular track (length, name, author, etc.).
	/// Must be freed after use.
	pub fn trackInfo(self: *const Emu, track: u32) Error!*Info {
		var info: ?*Info = null;
		const err = gme_track_info(self, &info, @intCast(track));
		return if (err != 0) toError(err) else info.?;
	}
	extern fn gme_track_info(*const Emu, *?*Info, c_int) Result;

	/// Disable automatic end-of-track detection and skipping of silence at beginning.
	pub fn ignoreSilence(self: *Emu, ignore: bool) void {
		gme_ignore_silence(self, @intFromBool(ignore));
	}
	extern fn gme_ignore_silence(*Emu, c_int) void;

	/// Number of voices used by currently loaded file.
	pub fn voiceCount(self: *const Emu) u32 {
		return @intCast(gme_voice_count(self));
	}
	extern fn gme_voice_count(*const Emu) c_int;

	/// Name of voice i, from 0 to `voiceCount()` - 1
	pub fn voiceName(self: *const Emu, i: u32) [:0]const u8 {
		const s = gme_voice_name(self, @intCast(i));
		return s[0..std.mem.len(s)];
	}
	extern fn gme_voice_name(*const Emu, c_int) [*:0]const u8;

	pub fn muteVoice(self: *Emu, index: u32, mute: bool) void {
		gme_mute_voice(self, @intCast(index), @intFromBool(mute));
	}
	extern fn gme_mute_voice(*Emu, c_int, c_int) void;

	/// Mute/unmute voice i, where voice 0 is first voice.
	pub fn muteVoices(self: *Emu, muting_mask: u32) void {
		gme_mute_voices(self, @intCast(muting_mask));
	}
	extern fn gme_mute_voices(*Emu, c_uint) void;
	...
	extern fn gme_disable_echo(*Emu, c_int) void;
	...
	extern fn gme_enable_accuracy(*Emu, c_int) void;
	...
	extern fn gme_multi_channel(*const Emu) c_int;
	...
	extern fn gme_load_file(*Emu, [*:0]const u8) Result;
	...
	extern fn gme_load_data(*Emu, *const anyopaque, c_long) Result;

	/// Load multiple single-track music files from memory into emulator.
	pub fn loadTracks(self: *Emu, data: [*]const u8, sizes: []usize) Error!void {
		const err = gme_load_tracks(self, data, @ptrCast(sizes.ptr), @intCast(sizes.len));
		if (err != 0)
			return toError(err);
	}
	extern fn gme_load_tracks(*Emu, [*]const u8, [*]c_long, c_uint) Result;

	/// Load music file using custom data reader function that will be called to
	/// read file data. Most emulators load the entire file in one read call.
	pub fn loadCustom(self: *Emu, func: Reader, data: []anyopaque) Error!void {
		const err = gme_load_custom(self, func, data.len, data.ptr);
		if (err != 0)
			return toError(err);
	}
	extern fn gme_load_custom(*Emu, Reader, c_long, *anyopaque) Result;

	/// Load m3u playlist file from memory (must be done after loading music).
	pub fn loadM3uData(self: *Emu, data: []const anyopaque) Error!void {
		const err = gme_load_m3u_data(self, data.ptr, data.len);
		if (err != 0)
			return toError(err);
	}
	extern fn gme_load_m3u_data(*Emu, *const anyopaque, c_long) Result;
};
