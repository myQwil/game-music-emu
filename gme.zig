...
fn toError(e: c_uint) Error {
	return error_list[e - err_offset];
}
...
pub const Type = opaque {
	...
	pub fn emu(self: *const Type, sample_rate: c_uint) Error!*Emu {
	...
	/// Returns an error if out of memory.
	pub fn emuMultiChannel(self: *const Type, sample_rate: c_uint) Error!*Emu {
	...
	pub fn fromFile(path: [*:0]const u8) Error!?*const Type {
		...
		return if (err != success) toError(err) else type_out;
	}
...
pub const Emu = opaque {
	...
	pub fn fromFile(path: [*:0]const u8, samplerate: c_uint) Error!*Emu {
		var self: ?*Emu = null;
		const err = gme_open_file(path, &self, samplerate);
		return if (err != success) toError(err) else self.?;
	}
	...
	pub fn fromData(data: []const u8, samplerate: c_uint) Error!*Emu {
		var self: ?*Emu = null;
		const err = gme_open_data(data.ptr, data.len, &self, samplerate);
		return if (err != success) toError(err) else self.?;
	}
	...
	pub fn startTrack(self: *Emu, index: c_uint) Error!void {
		const err = gme_start_track(self, index);
		if (err != success)
			return toError(err);
	}
	...
	pub fn seekSamples(self: *Emu, samples: c_uint) Error!void {
		const err = gme_seek_samples(self, samples);
		if (err != success)
			return toError(err);
	}
	...
	pub fn seekScaled(self: *Emu, msec: c_uint) Error!void {
		const err = gme_seek_scaled(self, msec);
		if (err != success)
			return toError(err);
	}
	...
	pub fn trackInfo(self: *const Emu, track: c_uint) Error!*Info {
		var info: ?*Info = null;
		const err = gme_track_info(self, &info, track);
		return if (err != success) toError(err) else info.?;
	}
	...
	pub fn loadTracks(
		...
		const err = gme_load_tracks(self, data, sizes.ptr, @intCast(sizes.len));
		if (err != success)
			return toError(err);
	}
	...
	pub fn loadM3uData(self: *Emu, data: []const u8) Error!void {
		const err = gme_load_m3u_data(self, data.ptr, @intCast(data.len));
		if (err != success)
			return toError(err);
	}
